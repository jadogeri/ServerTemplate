...
describe('EmailContext.getCompany() getCompany method', () => {
    ...
    describe('Edge Cases', () => {
        ...
        it('should return undefined when process.env.COMPANY is not set', () => {
            // This test checks the behavior when COMPANY is not set at all.
            delete process.env.COMPANY;
            const ctx = new EmailContext();
            // Type assertion is used because the method casts to string, but if undefined, it will return undefined.
            expect(ctx.getCompany()).toBeUndefined();
        });
